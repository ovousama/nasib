...
function StatusBadge({ status }: { status: MeetingRequest['status'] }) {
  const map = {
    ...
    cancelled: 'bg-[#FDFAF7] text-[#6B6B6B] border-[#EBEBEB]',
  }
  return (
  ...
  return (
    <div className="px-4 py-5 space-y-6">
      ...
      {!showForm ? (
        ...
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-[#EBEBEB] space-y-4">
          <h3 className="font-semibold text-[#1A1A1A]">New Meeting Request</h3>

          {/* Format */}
          <div>
            <p className="text-xs text-[#6B6B6B] mb-2 font-medium">Format</p>
                ...
                <button
                  ...
                  className={`py-2.5 rounded-xl text-sm font-medium border transition-colors ${
                    format === f
                      ...
                      : 'text-[#6B6B6B] border-[#EBEBEB] hover:border-[#AF4D98]'
                  }`}
                >
          ...
          <div className="space-y-3">
            <p className="text-xs text-[#6B6B6B] font-medium">Proposed times (up to 3)</p>
            {[
              ...
              <div key={label}>
                ...
                <input
                  ...
                  className="w-full border border-[#EBEBEB] rounded-xl px-3 py-2.5 text-sm text-[#1A1A1A] focus:outline-none focus:ring-2 focus:ring-[#AF4D98]"
                />
              </div>
            ))}
          </div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full border border-[#EBEBEB] rounded-xl px-3 py-2.5 text-sm text-[#1A1A1A] placeholder:text-[#9B9B9B] focus:outline-none focus:ring-2 focus:ring-[#AF4D98]"
            />
          </div>
      ...
      {pendingMeetings.length > 0 && (
        <div>
          <h3 className="font-semibold text-[#1A1A1A] mb-3">Awaiting Response</h3>
          <div className="space-y-3">
            {pendingMeetings.map(meeting => {
              ...
              return (
                <div key={meeting.id} className="bg-white rounded-2xl p-4 shadow-sm border border-[#EBEBEB]">
      ...
      {pastMeetings.length > 0 && (
        <div>
          <h3 className="font-semibold text-[#1A1A1A] mb-3">Past Meetings</h3>
          <div className="space-y-3">
      ...
      {meetings.length === 0 && !showForm && (
        <div className="bg-[#FDFAF7] rounded-2xl p-6 text-center border border-dashed border-[#EBEBEB]">
