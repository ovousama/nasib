        ...
        <table className="min-w-full text-sm">
          ...
          <tbody>
            ...
            {connections.map((conn, idx) => (
              <tr key={conn.id} className={`border-b border-[#EDE8E3] hover:bg-[#FAF4EE] transition-colors ${idx === connections.length - 1 ? 'border-b-0' : ''}`}>
                ...
                <td className="px-4 py-3">
                  <span className={`text-[11px] font-medium px-2.5 py-1 rounded-full ${conn.status === 'active' ? 'bg-[#F9F0F6] text-[#AF4D98]' : 'bg-[#FAF4EE] text-[#9B9B9B]'}`}>
                    {conn.status}
                  </span>
                </td>
                <td className="px-4 py-3 text-[#9B9B9B] text-xs whitespace-nowrap">{formatDate(conn.created_at)}</td>
                <td className="px-4 py-3 text-[#5C5C5C] text-center">{conn.meeting_count}</td>
                <td className="px-4 py-3">
                  {conn.status === 'active' && (
                    confirming === conn.id ? (
